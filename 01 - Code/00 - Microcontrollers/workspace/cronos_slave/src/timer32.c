...
//extern int TICKS,level;
extern volatile uint32_t cont_laps;
extern volatile uint32_t contador;
extern unsigned int esperar_PT_A,esperar_PT_B,Led_PT, Led_PM,start;
extern volatile uint32_t centesimas,lado,ultimo,lap[MAX_LAPS],fin,n_laps;
unsigned int leds_m=0, leds_p=0;
...
if(Led_PT)
{
	...
	if(leds_p==ESPERA)
	{
		leds_p=0;
		Led_PT=OFF;
		GPIOSetValue( PORT2, 6, 0 );	//Enciendo led placa de toque
	}
}
if(Led_PM)
{
	...
	if(leds_m==ESPERA)
	{
		leds_m=0;
		Led_PM=OFF;
		GPIOSetValue( PORT2, 7, 0 );	//Enciendo led placa de toque
	}
}
if(esperar_PT_A==ON)
{
	contador++;
	if(contador==ESPERA)	//Se cumplio el tiempo de espera. Se utiliza el tiempo de la PM
	{
		contador=0;		//Reseteo el contador
		esperar_PT_A=OFF;	//Bajo el flag de espera
		ultimo=lap[cont_laps];
		cont_laps++;	//Aumento el numero de laps
		lado=B;
		if(cont_laps==n_laps)
		{
			fin=ON;
		...
		GPIOIntDisable( PORT2, 2 );	//Dehabilito las interrupciones de la PT_A
		GPIOIntDisable( PORT2, 3 );	//Dehabilito las interrupciones de la PM_A
...
if(esperar_PT_B==ON)
{
	...
	{
		contador=0;		//Reseteo el contador
		esperar_PT_B=OFF;	//Bajo el flag de espera
		ultimo=lap[cont_laps];
		cont_laps++;	//Aumento el numero de laps
		lado=A;
		if(cont_laps==n_laps)
		...
		GPIOIntDisable( PORT2, 4 );	//Dehabilito las interrupciones de la PT_B
		GPIOIntDisable( PORT2, 5 );	//Dehabilito las interrupciones de la PM_B
