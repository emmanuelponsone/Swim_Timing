...
extern volatile uint32_t contador,espera,centesimas,bocina;
...
if(espera==ON)		//Indica que se presiono el boton de comienzo y hay que esperar a ver que pasa
{
	centesimas++;
	if(centesimas==HOLD)		//Espero 200mseg y habilito las interrupciones del boton de comienzo
	{
		GPIOIntEnable( PORT2, 2 );
	}
	if(centesimas==HOLD+200)	//Si sigue contando significa que no se volvio a presionar el boton.
	{
		espera=OFF;
		centesimas=0;
		if(comienzo==PC_OK)
		...
		contador=0;
		GPIOIntClear( PORT2, 2 );		//limpia por las dudas
		GPIOIntEnable( PORT2, 2 );		//habilita boton comienzo
		GPIOIntClear( PORT2, 4 );		//limpia por las dudas
		GPIOIntEnable( PORT2, 4 );		//habilita la interrupcion del boton de start

	}
if(espera==ANTIRREBOTE)		//Se presiono el boton de arranque de competencia. Antirrebote
{
	timer32_0_counter++;
	if(timer32_0_counter==2000)
	{
		timer32_0_counter=0;
